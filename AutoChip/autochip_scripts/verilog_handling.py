...
def generate_verilog_responses(conv, model_type, model_id="", num_candidates=1, base_url=None):
    if model_type == "ChatGPT":
        # 如果提供了 base_url，使用它；否则使用默认 OpenAI API
        if base_url:
        ...
        # 如果配置中指定了 base_url，优先使用配置的
        final_base_url = base_url if base_url else siliconflow_base_url
        model = lm.ChatGPT(model_id, base_url=final_base_url)
...
def get_iteration_model(iteration, mixed_model_config):
    ...
    for model_info in sorted_models:
        if iteration >= model_info['start_iteration']:
            ...
            base_url = model_info.get('base_url', None)  # 获取 base_url
            break
    #print(f"Family: {family}")
    #print(f"Model ID: {model_id}")
    #print(f"Base URL: {base_url}")
    return family, model_id, base_url
...
def verilog_loop(design_prompt, module, testbench, max_iterations, model_type, model_id="", num_candidates=5, outdir="", log=None, mixed_model_config={}):
    ...
    # 初始化 base_url，防止未启用混合模型时出现 UnboundLocalError
    base_url = None
    ...
    while not (success or timeout):
        ...
        if mixed_model_config:
            ...
            model_type, model_id, base_url = get_iteration_model(iterations, mixed_model_config)
        ...
        response_texts=generate_verilog_responses(conv, model_type, model_id, num_candidates=num_candidates, base_url=base_url)
