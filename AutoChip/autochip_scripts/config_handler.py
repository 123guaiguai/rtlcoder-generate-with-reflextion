...
def validate_mixed_model_config(mixed_model_config, max_iterations):
    ...
    for model_name, model_info in mixed_model_config.items():
        ...
        adjusted_config[model_name] = {
            ...
            "base_url": model_info.get('base_url', None)  # 支持 base_url 配置
        }
...
def parse_args_and_config(argv):
    ...
    try:
        opts, args = getopt.getopt(
            argv,
            "hp:n:t:i:f:m:k:o:l:c:",
            [
                "help", "prompt=", "name=", "testbench=", "iter=",
                "model-family=", "model-id=", "num-candidates=",
                "outdir=", "log=", "config="
            ]
        )
